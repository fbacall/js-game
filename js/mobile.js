class MobileEntity extends Entity {
    constructor (sprite, x, y, bTop, bRight, bBottom, bLeft, maxAcceleration, maxSpeed) {
        super(sprite, x, y, bTop, bRight, bBottom, bLeft);
    ...
    accelerate () {
        ...
        // Friction
        const frictionVector = normalise(this.velocity, friction);
    ...
    pathfindingAnimation () {
        if (this.path[0]) {
            const xDiff = this.path[0][X] - this.x;
            const yDiff = this.path[0][Y] - this.y;
